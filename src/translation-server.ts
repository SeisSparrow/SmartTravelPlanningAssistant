...
class TranslationServer {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new McpError(
          ...
          `Translation API error: ${error.response?.data.message ?? error.message}`
        );
      }
      throw error;
    }
  ...
  private async detectLanguage(args: any) {
    const { text } = args;
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new McpError(
          ErrorCode.InternalError,
          `Language detection API error: ${error.response?.data.message ?? error.message}`
        );
      }
      throw error;
    }
  ...
  private async getTravelPhrases(args: any) {
    ...
    const categoryPhrases = phrases[category as keyof typeof phrases] || phrases.basic;
  ...
  private async translateDestinationInfo(args: any) {
    ...
    const template = contentTemplates[contentType as keyof typeof contentTemplates] || contentTemplates.guide;
  ...
  private getMockTranslation(text: string, targetLanguage: string, sourceLanguage?: string): Translation {
