...
class CurrencyExchangeServer {
    ...
    async convertCurrency(args) {
        ...
        try {
            ...
            if (!rate) {
                throw new McpError(ErrorCode.InvalidParams, `Exchange rate not available for ${from} to ${to}`);
            }
            const exchangeRate = {
                from,
                to,
            ...
            if (axios.isAxiosError(error)) {
                throw new McpError(ErrorCode.InternalError, `Currency conversion error: ${error.response?.data.message ?? error.message}`);
            }
            throw error;
        }
    }
    async getDestinationCurrency(args) {
    ...
    async getTravelBudgetConversion(args) {
        const { budget, homeCurrency = 'USD', destinations } = args;
        const conversions = await Promise.all(destinations.map(async (dest) => {
            try {
...
//# sourceMappingURL=currency-server.js.map