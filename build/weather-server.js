...
class TravelWeatherServer {
    ...
    async getWeatherAlerts(args) {
        ...
        try {
            ...
            if (axios.isAxiosError(error)) {
                throw new McpError(ErrorCode.InternalError, `Weather alerts API error: ${error.response?.data.message ?? error.message}`);
            }
            throw error;
        }
    }
    async compareDestinationWeather(args) {
    ...
    processForecastData(data, startDate, endDate) {
    ...
    getMockWeatherData(destination, startDate, endDate) {
